...
module Timelog
  ...
  module WeeklyReport
    ...
    # Generate a report based on this weeks activities and write it to the
    # output stream.
    #
    ...
    # @param today [Time] Optionally, a day in the week to report on.
    #   Defaults to today.
    def self.render(timelog, output, today=nil)
      today ||= Time.now
      start_time, end_time = get_date_range(today)
      # activities = collect_activities(timelog, start_time, end_time)
    ...
    # Get the start and end times to report on based on the specified day.
    #
    # @param today [Time] A day in the week to report on.
    # @return [Time] A `start_time, end_time` 2-tuple.
    def self.get_date_range(today)
      current_day = today.wday
      current_day = 7 if current_day.zero?
      FIRST_WEEK_DAY - current_day
    end
